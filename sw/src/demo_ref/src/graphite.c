...
float vector_dot_product(vec3d* v1, vec3d* v2) { return MUL(v1->x, v2->x) + MUL(v1->y, v2->y) + MUL(v1->z, v2->z); }

float vector_length(vec3d* v) { return SQRT(vector_dot_product(v, v)); }
...
mat4x4 matrix_make_projection(int viewport_width, int viewport_height, float fov) {
    ...
    // projection matrix
    float near = FX(Z_NEAR);
    float far = FX(1000.0f);
    float aspect_ratio = FX((float)viewport_height / (float)viewport_width);
    float fov_rad = FX(1.0f / tanf(fov * 0.5f / 180.0f * 3.14159f));
...
float triangle_depth(triangle_t* tri) { return DIV(tri->p[0].z + tri->p[1].z + tri->p[2].z, FX(3.0f)); }
    ...
    for (size_t i = 0; i < model->mesh.nb_faces; ++i) {
        ...
        if (vector_dot_product(&normal, &vec_camera_ray) < FX(0.0f)) {
            ...
            for (int n = 0; n < nb_clipped_triangles; ++n) {
                ...
                float w = FX(viewport_width / 2);
                float h = FX(viewport_height / 2);
